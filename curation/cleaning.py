...
from curation.utils import GetReport, CleaningSMILES
...
class CleaningStage:
    ...
    def clean_salts(self,
        ...
        if sum(differ_after_clean_salt) > 0:
            contents += f'List of salts were cleaned: \n{index_of_salts}\n'
    ...
    def neutralize(self,
        ...
        if sum(differ_after_neutralize) > 0:
            contents += f'List of salts were cleaned:\n{index_of_targets}\n'
            contents += '\n'.join(f'{i}. {smiles}' for i, smiles in zip(index_of_targets, targets_in_data))
            contents += '\n'
    ...
    def clean_and_neutralize(self, check_validity: bool = True, output_dir_path: str = None,
        ...
        post_neutralized_smiles_data, differ_after_neutralizing, neutralizing_step_contents = (
            CleaningStage(post_salts_cleaned_smiles_data)
        ...
        summary = (f'Pre-cleaned SMILES data: {len(self.smiles_df)}\n'
        ...
        contents = '\n'.join([
            ...
            '----------',
            'SALTS CLEANING STEP:',
            cleaning_step_contents,
            '----------',
            'NEUTRALIZING STEP:',
            neutralizing_step_contents,
            '----------',
            'CLEANING COMPLETE!',
            'SUMMARY:',
            summary
