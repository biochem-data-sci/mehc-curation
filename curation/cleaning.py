...
class CleaningStage:
    ...
    def cl_salts(self,
        ...
        salts_cleaned = self.smi_df[smi_col[0]].p_apply(lambda x: CleaningSMILES(x, return_dif=True)
                                                        .cleaning_salts(return_is_null_smi=True))
        ...
        post_drop_missing_smi_str = (post_salts_cl_smi_data.drop(
            ...
            axis='index'))
        missing_smiles_cnt = len(post_salts_cl_smi_data) - len(post_drop_missing_smi_str)
        post_drop_missing_smi_str = post_drop_missing_smi_str.to_frame()

        format_data = {
            'salt_cleaning_input': len(self.smi_df),
            'desalted': sum(diff_after_cl_salt),
            'unprocessable': missing_smiles_cnt,
            'salt_cleaning_output': len(post_drop_missing_smi_str),
        }
        ...
        if param_deduplicate:
            post_drop_missing_smi_str, dup_idx_data, deduplicate_format_data = (
                deduplicate(post_drop_missing_smi_str,
                            validate=False,
                            get_output=False,
                            print_logs=False,
            ...
            post_drop_missing_smi_str = post_drop_missing_smi_str.reset_index(drop=True)
        ...
        if get_output:
            (GetReport(output_dir=output_dir,
             ...
             .create_csv_file(post_drop_missing_smi_str,
        ...
        if get_diff and return_format_data:
            return post_drop_missing_smi_str, diff_after_cl_salt, format_data
        elif get_diff and not return_format_data:
            return post_drop_missing_smi_str, diff_after_cl_salt
        elif return_format_data and not get_diff:
            return post_drop_missing_smi_str, format_data
        elif not get_diff and not return_format_data:
            return post_drop_missing_smi_str
    ...
    def neutralize(self,
        ...
        dup_idx_data = pd.DataFrame()
        if param_deduplicate:
            post_neutralized_smi_data, dup_idx_data, deduplicate_format_data = (
                deduplicate(post_neutralized_smi_data,
                            validate=False,
                            get_output=False,
                            print_logs=False,
                            show_dup_smi_and_idx=True,
            ...
            with open("./curation/template_report/deduplicate.txt", "r") as deduplicate_txt:
                template_report += deduplicate_txt.read()
            post_neutralized_smi_data = post_neutralized_smi_data.reset_index(drop=True)
        ...
        if get_output:
            (GetReport(output_dir=output_dir,
             ...
             .create_csv_file(post_neutralized_smi_data,
        ...
        if get_diff and return_format_data:
            return post_neutralized_smi_data, diff_after_neutralize, format_data
        elif get_diff and not return_format_data:
            return post_neutralized_smi_data, diff_after_neutralize
        elif return_format_data and not get_diff:
            return post_neutralized_smi_data, format_data
        elif not get_diff and not return_format_data:
            return post_neutralized_smi_data

    def cl_and_neutralize(self,
                          validate: bool = True,
                          output_dir: str = None,
                          print_logs: bool = True,
                          get_report: bool = False,
        ...
        post_salts_cl_smi_data, diff_after_cl_salt, cl_salt_format_data = self.cl_salts(
            validate=False,
            print_logs=False,
            get_diff=True,
            get_output=False,
        ...
        post_neutralized_smi_data, diff_after_neutralizing, neutralizing_format_data = (
            CleaningStage(post_salts_cl_smi_data)
            .neutralize(validate=False,
                        print_logs=False,
                        get_diff=True,
                        get_output=False,
                        return_format_data=True,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='This module is used to clean salts and neutralize SMILES data')
