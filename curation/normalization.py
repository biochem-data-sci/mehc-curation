...
class NormalizingStage:
    ...
    def detautomerize(
        ...
        post_detautomerized = self.smi_df[smi_col[0]].p_apply(
            lambda x: NormalizeSMILES(
                x, return_difference=True
            ).normalize_tautomer()
        )
        ...
        format_data = {
            ...
            "detautomerized": len(
                diff_after_detautomerized[
                    diff_after_detautomerized["diff"] == True
                ]
            ),
            "detautomerization_output": len(post_smi_df),
        }
        ...
        if get_output:
            (
                GetReport(
                    output_dir=output_dir, report_subdir_name="detautomerized"
                ).create_csv_file(
                    post_smi_df, csv_file_name="post_detautomerized.csv"
                )
            )
        ...
        if get_report:
            (
                GetReport(
                    ...
                    report_file_name="detautomerize_report.txt",
        ...
        if get_diff and return_format_data:
            ...
            return post_smi_df, diff_after_detautomerized
    ...
    def destereoisomerize(
        ...
        format_data = {
            ...
            "destereoisomerized": len(
                diff_after_destereoisomerized[
                    diff_after_destereoisomerized["diff"] == True
                ]
            ),
            "destereoisomerization_output": len(post_smi_df),
        }
        ...
        if get_report:
            (
                GetReport(
                    ...
                    report_file_name="normalize_stereoisomer_report.txt",
                    content=formatted_report,
                )
            )
        if get_diff and return_format_data:
            ...
            return post_smi_df, diff_after_destereoisomerized
        elif return_format_data and not get_diff:
            return post_smi_df, format_data
        else:
            return post_smi_df
    ...
    def complete_normalization(
        ...
        for the given SMILES dataframe.
        """
        ...
        (
            post_destereoisomerized_smi_data,
            diff_after_destereoisomerized,
            destereoisomerized_format_data,
        ...
        if param_deduplicate:
            ...
            with open(
                ...
                template_report += deduplicate_txt.read()
            post_detautomerized_smi_data = (
                post_detautomerized_smi_data.reset_index(drop=True)
            )
