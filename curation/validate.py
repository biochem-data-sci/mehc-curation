...
class ValidationStage:
    ...
    def validate_smi(self,
        ...
        self.smi_df['is_valid'] = self.smi_df[smi_col[0]].p_apply(lambda x: RemoveSpecificSMILES(x).is_valid())
        valid_smi = self.smi_df[self.smi_df['is_valid'] == True].copy()
        valid_smi.drop(columns=['is_valid'], inplace=True)
        invalid_smi = self.smi_df[self.smi_df['is_valid'] == False][smi_col[0]]
        idx_of_invalid_smi = self.smi_df[self.smi_df['is_valid'] == False].index.tolist()
        ...
        if param_deduplicate:
            valid_smi, dup_idx_data, rm_dup_format_data = (
                deduplicate(valid_smi,
                            validate=False,
                            get_output=False,
                            print_logs=False,
        ...
        if get_output:
            (GetReport(output_dir=output_dir,
             ...
             .create_csv_file(valid_smi,
                              csv_file_name='valid_smiles.csv'))

        if get_report:
            (GetReport(output_dir=output_dir,
                       report_subdir_name='check_valid_smiles')
             .create_report_file(report_file_name='validity_check.txt',
                                 content=formatted_report))
            (GetReport(output_dir=output_dir,
                       report_subdir_name='check_valid_smiles')
             .create_csv_file(invalid_smi,
                              csv_file_name='invalid_smiles.csv'))
            if param_deduplicate:
                (GetReport(output_dir=output_dir,
                           report_subdir_name='check_valid_smiles')
    ...
    def rm_mixtures(self,
        ...
        smi_col = self.smi_df.columns.tolist()
        if validate:
            self.smi_df, validate_format_data = self.validate_smi(get_output=False,
                                                                  print_logs=False,
        ...
        valid_smi.drop(columns=['is_valid'], inplace=True)
        invalid_smi = self.smi_df[self.smi_df['is_valid'] == True][smi_col[0]]
        idx_of_invalid_smi = self.smi_df[self.smi_df['is_valid'] == True].index.tolist()
        ...
        dup_idx_data = pd.DataFrame()
        if param_deduplicate:
            valid_smi, dup_idx_data, deduplicate_format_data = (
                deduplicate(valid_smi,
                            validate=False,
                            get_output=False,
                            print_logs=False,
        ...
        if get_output:
            (GetReport(output_dir=output_dir,
                       report_subdir_name='rm_mixtures')
             .create_csv_file(valid_smi,
                              csv_file_name='non_mixtures.csv'))

        if get_report:
            (GetReport(output_dir=output_dir,
                       report_subdir_name='rm_mixtures')
             .create_report_file(report_file_name='rm_mixtures.txt',
                                 content=formatted_report))
            (GetReport(output_dir=output_dir,
                       report_subdir_name='rm_mixtures')
             .create_csv_file(invalid_smi,
                              csv_file_name='mixtures.csv'))
            if param_deduplicate:
                (GetReport(output_dir=output_dir,
                           report_subdir_name='check_valid_smiles')
                 .create_csv_file(dup_idx_data,
    ...
    def rm_inorganics(self,
        ...
        if get_output:
            (GetReport(output_dir=output_dir,
             ...
             .create_csv_file(valid_smi,
                              csv_file_name='organics.csv'))

        if get_report:
            (GetReport(output_dir=output_dir,
                       report_subdir_name='rm_inorganics')
             .create_report_file(report_file_name='rm_inorganics.txt',
                                 content=formatted_report))
            (GetReport(output_dir=output_dir,
                       report_subdir_name='rm_inorganics')
             .create_csv_file(invalid_smi,
                              csv_file_name='inorganics.csv'))
            if param_deduplicate:
                (GetReport(output_dir=output_dir,
                           report_subdir_name='rm_inorganics')
                 .create_csv_file(dup_idx_data,
    ...
    def rm_organometallics(self,
        ...
        if get_output:
            (GetReport(output_dir=output_dir,
             ...
             .create_csv_file(valid_smi,
                              csv_file_name='non_organometallic.csv'))

        if get_report:
            (GetReport(output_dir=output_dir,
                       report_subdir_name='rm_organometallics')
             .create_report_file(report_file_name='rm_organometallics.txt',
                                 content=formatted_report))
            (GetReport(output_dir=output_dir,
                       report_subdir_name='rm_organometallics')
             .create_csv_file(invalid_smi,
                              csv_file_name='organometallic.csv'))
            if param_deduplicate:
                (GetReport(output_dir=output_dir,
                           report_subdir_name='rm_organometallics')
                 .create_csv_file(dup_idx_data,
    ...
    def complete_validation(self,
        ...
        self.smi_df, mixture_idx, rm_mixtures_format_data = self.rm_mixtures(validate=False,
                                                                             print_logs=False,
                                                                             get_invalid_smi_idx=True,
                                                                             get_output=False,
                                                                             param_deduplicate=False,
        ...
        self.smi_df, inorganic_idx, rm_inorganics_format_data = self.rm_inorganics(
            validate=False,
            print_logs=False,
            get_invalid_smi_idx=True,
            get_output=False,
        ...
        self.smi_df, organometallic_idx, rm_organometallics_format_data = self.rm_organometallics(
            validate=False,
            print_logs=False,
            get_invalid_smi_idx=True,
            get_output=False,
        ...
        dup_idx_data = pd.DataFrame()
        if param_deduplicate:
            self.smi_df, dup_idx_data, deduplicate_format_data = (
        ...
        if get_report:
            (GetReport(output_dir=output_dir,
                       report_subdir_name='complete_validation')
             .create_report_file(report_file_name='complete_validation.txt',
                                 content=formatted_report))
            if param_deduplicate:
                (GetReport(output_dir=output_dir,
                           report_subdir_name='complete_validation')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='This module is used to validate SMILES data')
    ...
    parser.add_argument('-c', '--choice', required=True, type=int, choices=range(1, 6),
    ...
    parser.add_argument('--validate', required=False, action='store_true', default=False,
                        help='Validate SMILES data before doing anything (except action 1, optional, default is false)')
